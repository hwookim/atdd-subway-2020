const validator = {
  path: {
    source: [(v) => !!v || '출발역 선택이 필요합니다.', (v) => v >= 0 || '출발역 ID는 0 이상의 양수이어야 합니다.'],
    target: [(v) => !!v || '도착역 선택이 필요합니다.', (v) => v >= 0 || '도착역 ID는 0 이상의 양수이어야 합니다.']
  },
  departureTime: {
    dayTime: [(v) => !!v || '오전, 오후 선택이 필요합니다.', (v) => (v === '오전' || v === '오후') || '오전, 오후 중에 선택해주세요.'],
    hour: [(v) => !!v || '시 선택이 필요합니다.', (v) => (v >= 1 && v <= 12) || '제대로 된 시간을 입력해주세요.'],
    minute: [(v) => !!v || '분 선택이 필요합니다.', (v) => (v >= 0 && v < 60) || '제대로 된 시간을 입력해주세요.']
  },
